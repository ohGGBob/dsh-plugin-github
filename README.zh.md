...
给 **DeepSeek Harness** 的智能体提供 GitHub 访问能力的插件：搜索仓库 / 代码 / issues / 用户、读取仓库文件与元信息，并且能**根据任务类型**推荐合适的 GitHub 资源。
...
在 DSH 的凭证设置里配置名为 `GITHUB_TOKEN` 的环境变量引用（或直接设置系统环境变量 `GITHUB_TOKEN` / `GH_TOKEN`），即可提升 API 速率限制。未配置也能正常使用（匿名限流：核心 60 次/小时，搜索 10 次/分钟）。
...
冒烟测试用一个假的 `ctx` 调用 `apply()`，确保三个工具的 parameter/output schema 都能通过 DSH 的 JSON Schema 子集校验，并端到端跑通 `github_catalog`（无需网络）。