...
const USER_AGENT = "dsh-plugin-github/0.2.0";
 ...
 *           description?: string, message?: string, dryRun?: boolean }} input
 * @returns {Promise<{ owner: string, repo: string, created: boolean,
 *           htmlUrl: string, dryRun: boolean, log: string[] }>}
 */
export async function publishToGitHub({ token, dir, repo: preferredRepo, private: isPrivate, description, message, dryRun }) {
  ...
  if (staged.status === 0) {
    ...
    const commit = runGit(git, absDir, ["commit", "-m", commitMsg], token);
    ...
    if (commit.status !== 0) {
      ...
      throw new Error(`github_push: commit failed${detail ? ` — ${detail}` : ""} (ensure git user.name / user.email are configured)`);
    }
  }
