 ...
 *   • `github_push`    — publish a local project directory to GitHub (create a
 *                        repository when missing, then commit and push to main).
 *
 ...
 * ./github.js, the curated library in ./catalog.js, and the publish logic in
 * ./push.js (all dependency-free); this module wires them up as model tools.
 *
...
import { publishToGitHub } from "./push.js";
...
const Config = z.object({
  ...
  /**
   * Default committer name used when `github_push` creates a commit. Only
   * used as a per-commit override (never persisted to the target `.git`); an
   * empty value falls back to the ambient global git `user.name`.
   */
  authorName: z.string().default("ohGGBob"),
  /**
   * Default committer email used when `github_push` creates a commit. As with
   * `authorName`, an empty value falls back to the ambient git `user.email`.
   */
  authorEmail: z.string().default("255260321+ohGGBob@users.noreply.github.com"),
});
...
const PROMPT_GUIDANCE = [
  ...
  "- Use the `github_push` tool to publish a local project directory to GitHub (point `path` at the folder; it creates a public/private repository when missing, commits, and pushes to `main`; it requires a `GITHUB_TOKEN`).",
...
/** Render a github_push result as model-facing text. */
function renderPush(_args, value) {
  const lines = Array.isArray(value.log) ? value.log : [];
  const headline = value.dryRun ? `GitHub publish (dry-run): ${value.owner}/${value.repo}` : `Published → ${value.htmlUrl}`;
  return [{ type: "text", text: [headline, "", ...lines].join("\n") }];
}
...
function apply(ctx, config) {
  ...
  const authorName = resolved.authorName || "ohGGBob";
  const authorEmail = resolved.authorEmail || "255260321+ohGGBob@users.noreply.github.com";
  ...
  ctx.tools.register(defineTool({
    name: "github_push",
    description: "Publish a local project directory to GitHub. Point `path` at a local folder; it will `git init` (when needed), commit, create a public or private repository under your account (when it does not already exist), and push to the `main` branch. Requires a GITHUB_TOKEN. The commit author defaults to the plugin's configured identity (never your secret token). Re-run on the same folder to push only the new local changes to the existing repository.",
    parameters: {
      path: { type: "string", required: true, description: "Absolute (or workspace-relative) path to the local project directory to publish." },
      private: { type: "boolean", default: false, description: "Create a private repository instead of a public one." },
      description: { type: "string", description: "Repository description. Defaults to package.json `description`, then the repository name." },
      message: { type: "string", description: "Commit message. Defaults to 'Initial commit'." },
      repo: { type: "string", description: "Override the repository name. Defaults to package.json `name`, then the directory name." },
      dryRun: { type: "boolean", default: false, description: "Report the plan only, without creating, committing, or pushing." },
    },
    output: {
      schema: {
        type: "object",
    ...
    async execute(args) {
      const token = await resolveToken(ctx, resolved.tokenEnv);
      if (!token) throw new Error("github_push requires a GITHUB_TOKEN (configure a credential named GITHUB_TOKEN, or set the GITHUB_TOKEN/GH_TOKEN environment variable).");
    ...
    presentCall: (args) => ({
      card: "generic",
      title: args.repo ? `Publish ${args.path} → ${args.repo}` : `Publish ${args.path}`,
      kind: "other",
      rawInput: args.path,
    }),
  }));
